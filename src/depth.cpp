...
void populate_marker_msg(visualization_msgs::MarkerArray &marker_array, std::string frame_id, std::vector<geometry_msgs::Point> objects_array, std::vector<std::string> &names){
    ...
    for(auto p : objects_array){
        ...
        if((p.x*p.x - p.y*p.y) > 0){ // remove markers close to camera
            ...
            marker_text.lifetime = ros::Duration(2.0);
...
int main(int argc, char** argv)
{
    ...
    ros::NodeHandle n;
    // ros::Rate r(10);
    ros::Publisher marker_pub           = n.advertise<visualization_msgs::MarkerArray>("object_markers", 1);
    ros::Publisher detection_publish    = n.advertise<object_detection_ros::arrbbox>("detection_bbox", 1);
    image_transport::ImageTransport it_(n);
    image_transport::Publisher image_pub_ = it_.advertise("/depth_image", 1);
	...
	while (ros::ok())
	{
        ...
        if (objects_array.size() > 0){
            ...
            populate_marker_msg(marker_array,"camera_link", objects_array, names);
